...
plotbicandentropy.function <- function(models) {
  results.melt <-
    melt(models$models, id = "NoClasses")
  results.melt
  # str.filename <-
...
X2018_spss_processed_variable_names_GOODTHINGS <-
  read_sav(
    "~/Dropbox/Research/Data and analyses/OfCom Data/2018-19/2018_spss_processed_variable_names GOODTHINGS.sav"
  )
...
X2018.uses.tmp <-
  subset(
    ...
    select = c(
      PoliticsIN13,
      GovernmentProcessIN13,
      CouncilTaxIN13,
      PublicServicesIN13,
      ApplyForJobIN13,
      LeisureIN13,
      ProductsIN13,
      CulturalActivitiesIN13,
      PayBillsIN13,
      SocialMediaRecodeIn21,
      SearchEngineRecodeIN45,
      YouTubeRecodeIN19,
      NetflixRecodeT1,
      GamingRecodeG1,
      AWTV2,
...
X2018.uses.data <- X2018.uses.tmp
X2018.uses.data.naomit <- na.omit(X2018.uses.data)
...
UserTypesLCAresults <- list()
##Call analysis with min 2 and max 10 classes
UserTypesLCAresults <-
  lcabestmodel.function(UsesBind, X2018.uses.data.naomit, 2, 10)
##Lattice plot of variable proportions by class
graphsforlca.function(UserTypesLCAresults$best$probs,
                      "Internet user groups")
##Store and view model selection as latex and htm file
UserTypesLCAresultsLatex <-
  xtable(UserTypesLCAresults$models)
View(UserTypesLCAresultsLatex)
str.filename <-
  paste(getwd(), "Modelresults.latex", sep = "")
print(UserTypesLCAresultsLatex, file = str.filename)
str.filename <-
  paste(getwd(), "Modelresults.htm", sep = "")
print(UserTypesLCAresultsLatex,
      type = "html",
      file = str.filename)
##Plot model selection variables
plotbicandentropy.function(UserTypesLCAresults$models)

UserTypesLCAresults$Classchisq <-
  chisquaretest.predictions.function(X2018.uses.data.naomit$AWTV3,
                                     UserTypesLCAresults$best$predclass)

UserTypesLCAresults$Agechisq <-
  chisquaretest.predictions.function(X2018.uses.data.naomit$AWTV2,
                                     UserTypesLCAresults$best$predclass)
